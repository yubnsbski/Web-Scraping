...
class SafeFetcher:
    ...
    def fetch_document(self, url: str, *, respect_robots: bool = True) -> FetchedDocument:
        ...
        ``respect_robots`` defaults to True (robots.txt is honored). Callers with
        an explicit, authorized reason (e.g. personal-use access to a source that
        blocks crawlers in robots.txt) may pass ``respect_robots=False`` to skip
        only the robots gate; SSRF protection, rate limiting, caching, and the
        configured User-Agent still apply.
        """
